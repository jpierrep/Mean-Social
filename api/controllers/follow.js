...
function deleteFollow(req,res){
    ...
    function getFollowingUsers(req,res){
       ...
       // Follow.find({user:userId}).populate('user followed').paginate(page,itemsPerPage,(err,follows,total)=>{ // popula la informacion del  user y el followed,trae los 2 objetos
       Follow.find({user:userId}).populate('user').paginate(page,itemsPerPage,(err,follows,total)=>{ // popula la informacion del  user solamente
    ...
    function getFollowedUsers(req,res){ //followers
        var userId=req.user.sub;
        //dar flexibilidad con el parametro opcional
        if(req.params.id && req.params.page){
            userId=req.params.id;
        ...
        Follow.find({followed:userId}).populate({path:'followed'}).paginate(page,itemsPerPage,(err,follows,total)=>{
            if(err) return    res.status(500).send({message:'Error en el servidor'});
            if(!follows) return res.status(500).send({message:'No te sigue ningun usuario'});
            return res.status(200).send({
                total:total,
                pages:Math.ceil(total/itemsPerPage),
                follows
...
function getMyFollows(req,res){
   ...
   //si recibo parametro followed me sacaria los usuarios que me estan siguiendo
   ...
   find.populate('user followed').exec((err,follows)=>{
        if(err) return    res.status(500).send({message:'Error en el servidor'});
        if(!follows) return res.status(500).send({message:'No sigues a ningun usuario'});
